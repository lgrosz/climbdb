...
\set parent `psql :'DBNAME' -Atc "select format('region %s %s %s', id, name, slug) from climb.regions union all select format('crag %s %s %s', id, name, slug) from climb.crags union all select format('sector %s %s %s', id, name, slug) from climb.sectors order by 1 " | fzf`
